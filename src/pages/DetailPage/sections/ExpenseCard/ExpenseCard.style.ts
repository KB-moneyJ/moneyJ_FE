...
const blink = keyframes`
  0% { opacity: 1; }
  50% { opacity: 0.8; box-shadow: 0 0 12px rgba(255, 254, 166, 0.9); }
  100% { opacity: 1; }
`;
...
export const Item = styled.div<{ $covered?: boolean; $purchased?: boolean }>`
  ...
  border: 0.4px solid
    ${({ $covered, $purchased }) =>
      $purchased ? '#9f9f9f' /* 짙은 보라 */ : $covered ? 'var(--color-text-highlight)' : 'white'};
  ...
  &:hover {
    border-color: ${({ $purchased, $covered }) =>
      $purchased ? '#9f9f9f' : $covered ? 'var(--color-text-highlight)' : 'rgba(255,255,255,0.4)'};
...
export const GoalButton = styled.button<{ $blink?: boolean }>`
  padding: 4px 8px;
  border-radius: 20px;
  ...
  font-size: 0.8rem;
  margin-right: 8px;
  animation: ${({ $blink }) => ($blink ? blink : 'none')} 1.5s infinite ease-in-out;
